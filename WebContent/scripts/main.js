(function() {
	...
	function register() {
		...
		function(res) {
			...
			// successfully registered, redirect to index.html
			if (result.status === 'OK') {
	...
	function showRegisterError() {
		$('register-error').innerHTML = 'Username already exists';
	}
	 ...
	 * /Stylist/boutiques?user_id=1111&lat=37.38&lon=-122.08
	 */
	function loadNearbyRestaurants() {
		console.log('loadNearbyBoutiques');
		...
		// display loading message
		showLoadingMessage('Loading nearby boutiques...');
		...
		function(res) {
			...
			if (!restaurants || restaurants.length === 0) {
				showWarningMessage('No nearby boutiques.');
			} else {
		...
		// failed callback
		function() {
			showErrorMessage('Cannot load nearby boutiques.');
		});
	}
	...
	function loadFavoriteRestaurants(event) {
		...
		// display loading message
		showLoadingMessage('Loading favorite boutiques...');

		// make AJAX call
		ajax('GET', url + '?' + params, req, function(res) {
			var restaurants = JSON.parse(res);
			if (!restaurants || restaurants.length === 0) {
				showWarningMessage('No favorite boutique.');
			} else {
				listRestaurants(restaurants);
			}
		}, function() {
			showErrorMessage('Cannot load favorite boutiques.');
		});
	}
	...
	function loadRecommendedRestaurants() {
		...
		// display loading message
		showLoadingMessage('Loading recommended boutiques...');
		...
		ajax(
				...
				function(res) {
					var restaurants = JSON.parse(res);
					if (!restaurants || restaurants.length === 0) {
						showWarningMessage('No recommended boutique. Make sure you have favorites.');
					} else {
						listRestaurants(restaurants);
					}
				},
				// failed callback
				function() {
					showErrorMessage('Cannot load recommended boutiques.');
				});
	}
